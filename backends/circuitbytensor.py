...
class CircuitByTensor:
    ...
    def graph_to_tensorCircuit(self, params, graph, original_e=None):
    ...
    def expectation_calculation_parallel(self):
        ...
        circ_res.append(pool.map(self.get_sample_expectation, list(self._subg_to_circuit.values()), chunksize=1))
        pool.terminate()
        pool.join()
        res = sum(circ_res[0])
        print("Expectation value of original graph (tensor calculation) is: ", res)
        return res
    def get_result_statevector(self, param, graph):
        _, tensor = self.graph_to_tensorCircuit(params, graph)
        return tensor.psi

    def get_result_counts(self, params, graph):
        _, tensor = sefl.graph_to_tensorCircuit(params, graph)
        binary_list = []
        hist = dict()
        for i in tensor.sample(1024):
            binary_list.append(i)
        binary_set = set(binary_list)
        for key in binary_set:
            hist[key] = binary_list.count(key)
