...
if __name__ == '__main__':
    ...
    from Qcover.applications import MaxCut, GraphColoring
    mxt = MaxCut(g)
    ising_g, shift = mxt.run()

    # gc = GraphColoring(node_num=5, node_degree=2, color_num=3)
    # ising_g, shift = gc.run()

    # ising_g = nx.Graph()
    # for i in range(6):
    #     # ising_g.add_node(i, weight=0)
    ...
    optg = GradientDescent(maxiter=100, tol=1e-3, learning_rate=0.05)  #
    optc = COBYLA(initial_point=[0.5, 0.5], options={'tol': 1e-3, 'disp': True})  # 'maxiter': 30,
    opti = Interp(optimize_method="COBYLA", options={'tol': 1e-3, 'disp': False})
    optf = Fourier(p=p, q=2, r=0, alpha=0.6, optimize_method="COBYLA", options={'tol': 1e-3, 'disp': False})
    ...
    # print(qc_c._backend._expectation_path)
    print(qc_c._backend._element_expectation)
